interface Crossing {
    crossingId: number; // Eindeutige ID der Schranke
    state: boolean; // Zustand der Schranke (true = unten, false = oben)
    disabled?: boolean;
}
...
type IncomingMessage = Crossing | Crossing[] | TrafficData | TrafficData[] | Countdown;

const timeOptions = { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' } as const;
let socket: WebSocket; // WebSocket-Objekt
...
function connectWebSocket() {
    ...
    socket.addEventListener('message', (event) => {
        ...
        let data = JSON.parse(event.data) as IncomingMessage; // Nachricht parsen
           
        if (Array.isArray(data)) {
            const firstEntry = data[0];
            if ('state' in firstEntry) {
                console.log(data);
                data = data as Crossing[];
                console.log('Receiving initial-data.');
                updateCrossingList(data);
            } else if ('id' in firstEntry) {
                data = data as TrafficData[];
...
function addTrafficEntry(data: TrafficData) {
...
function startCrossingCountdown(crossingId: number, time: number) {
    const countdownDiv = document.getElementById("cd-" + crossingId) as HTMLDivElement;
    countdownDiv.style.display = "block";
    
    const spanElement = document.getElementById("cdt-" + crossingId) as HTMLSpanElement;
...
function toggleCrossing(crossingId: number) {
...
function setTextState(element: HTMLSpanElement, crossingId: number, state: boolean) {
    element.textContent = `Übergang ${crossingId} - Schranken ${state ? 'Oben' : 'Unten'}`;
}

function updateCrossingList(crossings: Crossing[]): void {
...
function updateSingleCrossing(crossingId: number, newState: boolean): void {
    ...
    setTextState(crossingText as HTMLSpanElement, crossingId, newState);
    setButtonState(crossingButton as HTMLButtonElement, newState);
}
